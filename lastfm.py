...
class artists:
    ...
    scrapped_tags= pd.read_csv('..../scrapedtags.csv').drop('Unnamed: 0',axis=1)    
        ...
        for url in urls:
            ...
            if tags != None:
               for tag in tags:
                    ...
                    taggs.append(url)
                    taggs.append(tagged)
        #Save sracped tags to the scrapped_tags dataframe            
    ...
    def create_ii_matrix():
        ...
        untagged= pd.concat([artists.df_artists['id'],artists.df_tags['artistID']]).drop_duplicates(keep=False) 
        df_untagged= artists.df_artists.loc[artists.df_artists['id'].isin(untagged.values)]
        df_untagged_tagged= pd.merge(df_untagged,artists.scrapped_tags[['url','tag']],how='left').fillna('NA').set_index('tag')
